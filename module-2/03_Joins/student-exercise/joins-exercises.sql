...
SELECT f.title, COUNT(rental_id) AS numberrented
  FROM category c 
  JOIN film_category fc
  ON c.category_id = fc.category_id
  JOIN film f
...
GROUP BY f.title
ORDER BY numberrented DESC
LIMIT 5;

-- 19. The top 10 actors ranked by number of rentals of films starring that actor 
-- (#1 should be “GINA DEGENERES” with 753 rentals and #10 should be “SEAN GUINESS” with 599 rentals)
SELECT a.first_name || ' ' || a.last_name AS full_name, COUNT(r.rental_id) AS timesrented
  FROM actor a
  JOIN film_actor fa
...
SELECT a.first_name || ' ' || a.last_name  AS actor_name, COUNT(r.rental_id) AS timesrented
  FROM actor a 
 JOIN film_actor fa
  ON a.actor_id = fa.actor_id
 JOIN film f
